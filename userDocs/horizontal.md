...
`Horizontal` places child renderables from left to right inside a fixed terminal-cell frame.
...
Children are placed at the container's `y` coordinate and the next available `x` coordinate. Child frames provide their desired width and height, but their final position is controlled by the horizontal container.
